...
Use this plugin for enterprise-specific runtime context that should not live in the AgentSeek core package. The first capability is employee identity enrichment:
...
The plugin also exposes a disabled-by-default `system_prompt()` hook. Set `AGENTSEEK_ENTERPRISE_IDENTITY_SYSTEM_PROMPT=true` only when the model adapter consumes Bub system prompts.
...
- Long-term memory, short-term memory persistence, and WeCom callback handling are intentionally separate modules.
