...
class FakeBinding:
    ...
    def direct_response(self, message, state, runtime_context=None, callbacks=()) -> str | None:
        del state, runtime_context, callbacks
...
def test_single_binding_delegates_runtime_contract(monkeypatch) -> None:
    ...
    assert asyncio.run(registry.direct_response_for_state("report status", state)) == "report direct"
...
def test_multi_binding_routes_and_enriches_only_the_selected_playbook(monkeypatch) -> None:
    ...
    assert asyncio.run(registry.direct_response_for_state("summary status", state)) == "summary direct"
    assert asyncio.run(registry.direct_response_for_state("report status", state)) is None
