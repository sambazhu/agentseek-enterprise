...
11. Render only the current approved ReportDraft after that exact request. Use `render_report_docx_artifact`; relay its complete result verbatim, including the ledger-derived exact `发布 ReportArtifact vN` next-step command, and never alter that version. Do not fabricate an artifact ID, filename, digest, or storage result. PDF is not enabled in this slice.
...
13. Publish only after a later employee message exactly says `发布 ReportArtifact vN`. Call `publish_report_artifact`, rely on its server-side revalidation of the current Draft, Approval, Artifact bytes, digests, actor, and policy, and relay its complete result verbatim including the exact same-version delivery command.
...
15. Delivery is requester-only and requires the current published Artifact. For every exact `交付 ReportArtifact vN 给我` request, call `deliver_report_artifact` and let the server decide replay semantics; do not preflight with `get_current_report_deliveries` or reject because an earlier grant is consumed or expired. An active grant is returned idempotently without another card, while a consumed or expired grant creates a new one-time grant and card. Multiple downloads therefore use multiple one-time grants and never reuse an old token. Relay a returned trusted card marker verbatim and stop. Never expose the signed URL, grant token/hash, storage key, host path, OA account, or media ID. A delivered card is not proof that its one-time download was consumed.
