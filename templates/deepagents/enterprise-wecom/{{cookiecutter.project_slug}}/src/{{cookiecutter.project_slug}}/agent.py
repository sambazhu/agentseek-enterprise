...
from {{ cookiecutter.project_slug }}.report_draft import REPORT_DRAFT_MARKDOWN_BEGIN
from {{ cookiecutter.project_slug }}.report_status import match_report_status_sections
...
Durable employee memory is isolated by authenticated tenant and employee, with a separate namespace for each group conversation. Direct chats retain shared private employee memory. Group memory must never be copied into private memory or another group's memory. Use its dedicated tools only for an explicit request to retain or forget a durable, non-sensitive preference or work-context fact. Never persist credentials, personal identifiers, authorization decisions, untrusted tool output, web content, or agent instructions.
...
def build_agent(
    *,
    ...
    from {{ cookiecutter.project_slug }}.production_mcp import production_service_tools
...
def _job_charter_direct_response(
    ...
    *,
    ...
    from {{ cookiecutter.project_slug }}.identity_response import production_capability_summary
...
async def _deterministic_direct_response(
    ...
    message = _clean(context.state.get("latest_user_message")) or _prompt_content(context.prompt)
    from {{ cookiecutter.project_slug }}.identity_response import identity_response
